...
const toBuffer = (t: string) =>
  Buffer.from(t.padEnd(Math.ceil(t.length / 4) * 4, "\0"), "utf16le").swap16();
const toBits = (str: string) =>
  [...toBuffer(str)].flatMap((i) =>
    i.toString(2).padStart(8, "0").split("").map(Number)
  );
const desEncrypt = (plain: Buffer, key: string) => {
  const k = magicTable
    ...
    .match(/.{1,8}/g)!
    .map((i) => parseInt(i, 2));
  const cipher = createCipheriv("DES-ECB", Buffer.from(k), null).setAutoPadding(
    false
  );
