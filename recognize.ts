...
const transpose = (data: Buffer, width: number) => {
  return [...new Array(width).keys()].map((i) =>
    [...data].filter((_, j) => j % width === i)
  );
};
...
export const recognize = async (ab: ArrayBuffer) => {
  ...
  const digits = transpose(
    await sharp(digitsImage).toColourspace("b-w").raw().toBuffer(),
    180
  );
